 ...
 public:
  ...
  int widthOfBinaryTree_bfs(TreeNode* root) {
    ...
    while (!arr.empty()) {
      ...
      for (auto& [node, index] : arr) {
        if (node->left) temp.emplace_back(node->left, index * 2);
        if (node->right) temp.emplace_back(node->right, index * 2 + 1);
      }
  ...
  int widthOfBinaryTree_dfs(TreeNode* root) {
    ...
    function<ULL(TreeNode*, int, ULL)> dfs = [&](TreeNode* node, int depth,
      ...
      return max(static_cast<ULL>(index - levelmin[depth] + 1),
                 max(dfs(node->left, depth + 1, index * 2),
